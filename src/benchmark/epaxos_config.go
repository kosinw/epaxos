...
type epaxosConfig struct {
	mu           sync.Mutex
	t            *testing.T
	net          *labrpc.Network
	n            int
	kvservers    []*epaxoskv.KVServer
	saved        []*epaxos.Persister
	endnames     [][]string // names of each server's sending ClientEnds
	clerks       map[*epaxoskv.Clerk][]string
	nextClientId int
...
func (cfg *epaxosConfig) checkTimeout() {
...
func (cfg *epaxosConfig) LogSize() int {
	logsize := 0
	for i := 0; i < cfg.n; i++ {
...
func (cfg *epaxosConfig) connectUnlocked(i int, to []int) {
...
func (cfg *epaxosConfig) connect(i int, to []int) {
	cfg.mu.Lock()
	defer cfg.mu.Unlock()
...
// caller must hold cfg.mu
func (cfg *epaxosConfig) disconnectUnlocked(i int, from []int) {
...
func (cfg *epaxosConfig) disconnect(i int, from []int) {
	cfg.mu.Lock()
	defer cfg.mu.Unlock()
	cfg.disconnectUnlocked(i, from)
}

func (cfg *epaxosConfig) All() []int {
	all := make([]int, cfg.n)
	for i := 0; i < cfg.n; i++ {
		all[i] = i
	}
...
func (cfg *epaxosConfig) ConnectAll() {
	cfg.mu.Lock()
	defer cfg.mu.Unlock()
	for i := 0; i < cfg.n; i++ {
...
func (cfg *epaxosConfig) partition(p1 []int, p2 []int) {
	cfg.mu.Lock()
	defer cfg.mu.Unlock()
...
func (cfg *epaxosConfig) makeClient(to []int) Clerk {
	cfg.mu.Lock()
	defer cfg.mu.Unlock()
	...
	ck := epaxoskv.MakeClerk(random_handles(ends))
...
func (cfg *epaxosConfig) deleteClient(ck Clerk) {
	cfg.mu.Lock()
	defer cfg.mu.Unlock()

	v := cfg.clerks[ck.(*epaxoskv.Clerk)]
	for i := 0; i < len(v); i++ {
		os.Remove(v[i])
	}
	delete(cfg.clerks, ck.(*epaxoskv.Clerk))
}

// caller should hold cfg.mu
func (cfg *epaxosConfig) ConnectClientUnlocked(ck Clerk, to []int) {
	// log.Printf("ConnectClient %v to %v\n", ck, to)
	endnames := cfg.clerks[ck.(*epaxoskv.Clerk)]
	for j := 0; j < len(to); j++ {
		s := endnames[to[j]]
...
func (cfg *epaxosConfig) ConnectClient(ck Clerk, to []int) {
	cfg.mu.Lock()
	defer cfg.mu.Unlock()
	cfg.ConnectClientUnlocked(ck, to)
}
...
// caller should hold cfg.mu
func (cfg *epaxosConfig) DisconnectClientUnlocked(ck Clerk, from []int) {
...
func (cfg *epaxosConfig) DisconnectClient(ck Clerk, from []int) {
	cfg.mu.Lock()
	defer cfg.mu.Unlock()
...
func (cfg *epaxosConfig) ShutdownServer(i int) {
	cfg.mu.Lock()
	defer cfg.mu.Unlock()
...
func (cfg *epaxosConfig) StartServer(i int) {
	cfg.mu.Lock()
	...
	cfg.kvservers[i] = epaxoskv.StartKVServer(ends, i, cfg.saved[i])
...
func (cfg *epaxosConfig) Leader() (bool, int) {
	cfg.mu.Lock()
	defer cfg.mu.Unlock()
...
func (cfg *epaxosConfig) make_partition() ([]int, []int) {
	_, l := cfg.Leader()
...
var ncpu_epaxos_once sync.Once

func make_epaxos_config(t *testing.T, n int, unreliable bool) Config {
	ncpu_epaxos_once.Do(func() {
		if runtime.NumCPU() < 2 {
	...
	cfg.n = n
	cfg.kvservers = make([]*epaxoskv.KVServer, cfg.n)
	cfg.saved = make([]*epaxos.Persister, cfg.n)
	cfg.endnames = make([][]string, cfg.n)
	cfg.clerks = make(map[*epaxoskv.Clerk][]string)
...
func (cfg *epaxosConfig) rpcTotal() int {
...
func (cfg *epaxosConfig) begin(description string) {
	fmt.Printf("%s ...\n", description)
	cfg.t0 = time.Now()
...
func (cfg *epaxosConfig) configureWideArea() {
