...
func (cfg *raftConfig) checkTimeout() {
...
func (cfg *raftConfig) LogSize() int {
	logsize := 0
	for i := 0; i < cfg.n; i++ {
...
func (cfg *raftConfig) SnapshotSize() int {
	snapshotsize := 0
	for i := 0; i < cfg.n; i++ {
...
func (cfg *raftConfig) connectUnlocked(i int, to []int) {
...
func (cfg *raftConfig) connect(i int, to []int) {
	cfg.mu.Lock()
	defer cfg.mu.Unlock()
...
// caller must hold cfg.mu
func (cfg *raftConfig) disconnectUnlocked(i int, from []int) {
...
func (cfg *raftConfig) disconnect(i int, from []int) {
	cfg.mu.Lock()
	defer cfg.mu.Unlock()
	cfg.disconnectUnlocked(i, from)
}

func (cfg *raftConfig) All() []int {
	all := make([]int, cfg.n)
	for i := 0; i < cfg.n; i++ {
		all[i] = i
	}
...
func (cfg *raftConfig) ConnectAll() {
	cfg.mu.Lock()
	defer cfg.mu.Unlock()
	for i := 0; i < cfg.n; i++ {
...
func (cfg *raftConfig) partition(p1 []int, p2 []int) {
	cfg.mu.Lock()
	defer cfg.mu.Unlock()
...
func (cfg *raftConfig) makeClient(to []int) Clerk {
	cfg.mu.Lock()
	defer cfg.mu.Unlock()
...
func (cfg *raftConfig) deleteClient(ck Clerk) {
	cfg.mu.Lock()
	defer cfg.mu.Unlock()

	v := cfg.clerks[ck.(*kvraft.Clerk)]
	for i := 0; i < len(v); i++ {
		os.Remove(v[i])
	}
	delete(cfg.clerks, ck.(*kvraft.Clerk))
}

// caller should hold cfg.mu
func (cfg *raftConfig) ConnectClientUnlocked(ck Clerk, to []int) {
	// log.Printf("ConnectClient %v to %v\n", ck, to)
	endnames := cfg.clerks[ck.(*kvraft.Clerk)]
	for j := 0; j < len(to); j++ {
		s := endnames[to[j]]
...
func (cfg *raftConfig) ConnectClient(ck Clerk, to []int) {
	cfg.mu.Lock()
	defer cfg.mu.Unlock()
	cfg.ConnectClientUnlocked(ck, to)
}
...
// caller should hold cfg.mu
func (cfg *raftConfig) DisconnectClientUnlocked(ck Clerk, from []int) {
...
func (cfg *raftConfig) DisconnectClient(ck Clerk, from []int) {
	cfg.mu.Lock()
	defer cfg.mu.Unlock()
...
func (cfg *raftConfig) ShutdownServer(i int) {
	cfg.mu.Lock()
	defer cfg.mu.Unlock()
...
func (cfg *raftConfig) StartServer(i int) {
	cfg.mu.Lock()
...
func (cfg *raftConfig) Leader() (bool, int) {
	cfg.mu.Lock()
	defer cfg.mu.Unlock()
...
func (cfg *raftConfig) make_partition() ([]int, []int) {
	_, l := cfg.Leader()
...
var ncpu_raft_once sync.Once

func make_raft_config(t *testing.T, n int, unreliable bool) Config {
	ncpu_raft_once.Do(func() {
		if runtime.NumCPU() < 2 {
...
func (cfg *raftConfig) rpcTotal() int {
...
func (cfg *raftConfig) begin(description string) {
	fmt.Printf("%s ...\n", description)
	cfg.t0 = time.Now()
