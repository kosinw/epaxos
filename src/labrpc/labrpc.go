...
func (e *ClientEnd) Call(svcMeth string, args interface{}, reply interface{}) bool {
	...
	// fmt.Printf("[labrpc] request has been sent \n")

	//
	// wait for the reply.
	//
	rep := <-req.replyCh
	// fmt.Printf("[labrpc] received rep %v \n", rep)
	if rep.ok {
...
type Network struct {
	...
	extraDelay     map[interface{}]time.Duration // endname,servername -> duration
	endCh          chan reqMsg
	done           chan struct{} // closed when Network is cleaned up
...
func (rn *Network) getExtraDelay(endname interface{}) time.Duration {
	...
	if dur, ok := rn.extraDelay[endname]; ok {
		return dur
	}
